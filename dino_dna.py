...
if torch.cuda.is_available():
    ...
    device_student = torch.device("cuda:0")  # Output device for DataParallel
else:
    device_teacher = device_student = torch.device("cpu")
    device_ids = []
...
def train_dino(model, teacher_model, dataloader, optimizer, num_epochs, 
               n_subseq, m_masked, fraction, mask_prob, mask_token_id, pad_token_id, 
               l, m, tps, tpt):
    center = torch.zeros(model.module.projection_head[-1].out_features if isinstance(model, nn.DataParallel) else 
                         model.projection_head[-1].out_features, device=device_student)
    ...
    for epoch in range(num_epochs):
        ...
        for batch in tqdm(dataloader, desc=f"Epoch {epoch+1}/{num_epochs}", leave=False):
            try:
                ...
                subseq_views = generate_subsequence_views(global_view, n_subseq, fraction, 
                                                        model.module.max_len if isinstance(model, nn.DataParallel) else model.max_len, 
                                                        pad_token_id)
                masked_views = generate_masked_views(global_view, m_masked, mask_prob, mask_token_id, 
                                                    model.module.max_len if isinstance(model, nn.DataParallel) else model.max_len, 
                                                    pad_token_id)
                student_views = [global_view] + subseq_views + masked_views
                n_views = len(student_views)
                merged_views = torch.cat(student_views, dim=0)
                
                student_outputs = model(merged_views)
                expanded_teacher_output = teacher_output.repeat_interleave(n_views, dim=0)
                loss = dino_loss(student_outputs, expanded_teacher_output, tps, tpt, center)
                
                s_probs = F.softmax(student_outputs, dim=1)
                s_entropy = - (s_probs * torch.log(s_probs + 1e-7)).sum(dim=1).mean().item()
                normalized_student_entropy = s_entropy / max_entropy
                
                if torch.isnan(loss):
                    optimizer.zero_grad()
                    del (global_view, subseq_views, masked_views, student_views, 
                         merged_views, student_outputs, expanded_teacher_output, loss)
                    torch.cuda.empty_cache()
                    gc.collect()
                    continue
                ...
                if "out of memory" in str(e):
                    print("CUDA OOM error encountered, skipping batch.")
                    optimizer.zero_grad()
                    torch.cuda.empty_cache()
                    gc.collect()
                    continue
        ...
        avg_teacher_entropy = total_teacher_entropy / batch_count if batch_count > 0 else float('nan')
        avg_student_entropy = total_student_entropy / batch_count if batch_count > 0 else float('nan')
        print(f"Epoch {epoch+1}/{num_epochs}, Avg Loss: {avg_loss:.3f}, Avg T_Std: {avg_teacher_std:.3f}, "
...
if __name__ == "__main__":
    ...
    dataset = DNADataset(min_length=max_len_seq//2, max_length=max_len_seq, 
                         context_length=context_length, dataset_size=1000)
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True)
    ...
    teacher_model.load_state_dict(model.module.state_dict() if isinstance(model, nn.DataParallel) else model.state_dict())
