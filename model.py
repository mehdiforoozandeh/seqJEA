...
VOCAB_SIZE = 4096  # Default vocabulary size
...
class UnifiedDNATransformer(nn.Module):
    ...
    def __init__(self, model_type="alibi", **kwargs):
        ...
        if self.model_type == "alibi":
            ...
            self.model = self.DNATransformer_Relative(**kwargs)
        elif self.model_type == "sinusoidal":
            self.model = self.DNATransformer_Sinusoidal(**kwargs)
        elif self.model_type in ["bert2", "dnabert2"]:
            self.model = self.TransformerEncoder(**kwargs)
        else:
            raise ValueError("Invalid model_type. Choose from 'alibi', 'relative', 'sinusoidal', or 'bert2'.")
    ...
    class ALiBiTransformerEncoderLayer(nn.Module):
        ...
        def __init__(self, embed_dim, num_heads, dim_feedforward, dropout=0.1, max_len=512):
            super().__init__()
            self.self_attn = UnifiedDNATransformer.ALiBiMultiheadAttention(
                embed_dim, num_heads, dropout=dropout, max_len=max_len
            )
            self.linear1 = nn.Linear(embed_dim, dim_feedforward)
            self.dropout = nn.Dropout(dropout)
            self.linear2 = nn.Linear(dim_feedforward, embed_dim)
    ...
    class RelativeTransformerEncoderLayer(nn.Module):
        ...
        def __init__(self, embed_dim, num_heads, dim_feedforward, dropout=0.1, max_len=512):
            super().__init__()
            self.self_attn = UnifiedDNATransformer.RelativeMultiheadAttention(
                embed_dim, num_heads, dropout=dropout, max_len=max_len
            )
            self.linear1 = nn.Linear(embed_dim, dim_feedforward)
            self.dropout = nn.Dropout(dropout)
    ...
    class DNATransformer_Relative(nn.Module):
        ...
        def __init__(self, vocab_size=VOCAB_SIZE, embed_dim=256, num_layers=4, num_heads=4,
            ...
            self.embedding = nn.Embedding(vocab_size, embed_dim)
            self.layers = nn.ModuleList([
                UnifiedDNATransformer.RelativeTransformerEncoderLayer(
                    embed_dim, num_heads, dim_feedforward, dropout, max_len
                ) for _ in range(num_layers)
            ])
            self.projection_head = nn.Sequential(
...
if __name__ == "__main__":
    ...
    for variant in ["alibi", "relative", "sinusoidal", "bert2"]:
        ...
        print(f"Output shape for {variant}: {output.shape}")
